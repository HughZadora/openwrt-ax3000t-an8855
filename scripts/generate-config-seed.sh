#!/bin/bash
# ============================================================
# Generate .config seed for the OpenWrt AX3000T-AN8855 build
# Called by scripts/build-firmware during channel preparation.
# ============================================================
...
# XZ-compressed initramfs: the stock-U-Boot single-UBI path enforces a
# 26 MiB FIT limit, and XZ is field-proven on this exact boot path (main
# channel). Pinned explicitly because a blind first defconfig can bake in
# a channel default of NONE (observed on 24.10: 63.8MB uncompressed).
...
# Build OpenClash as a standalone package, but do not include it in the
# firmware image (format follows the channel feed: .apk on main, .ipk on
# openwrt-24.10).
